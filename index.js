...
function createDictionary(entries) {
  const dic = new Map(); // Створення порожнього словника
  entries.forEach(([key, value]) => dic.set(key, value));
  return dic;
...
function convertMapToObject(map) {
  const obj = {}; // Створення порожнього об'єкту
  map.forEach((value, key) => (obj[key] = value)); // Перебираємо ключі та значення в словнику
  // Додаємо ключ та значення до об'єкту за допомогою квадратних дужок
...
function setValue(dictionary, key, value) {
  ...
  return dictionary; // Повертаємо змінений словник
}
...
function getKeysStartingWith(dictionary, letter) {
  let keysStartingWith = [];

  for (let key of dictionary.keys()) {
    if (key.startsWith(letter)) {
      keysStartingWith.push(key);
    }
  }
...
function sortByValues(dictionary) {
  const sortedDictionary = new Map(
    [...dictionary.entries()].sort((a, b) => b[1] - a[1])
  );
